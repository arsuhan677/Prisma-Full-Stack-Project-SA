...
export default function Cart() {
  ...
  const shippingCost =
    selectedCartItems.length > 0 ? shippingSelected.price : 0;
  const totalCost = selectedSubtotal + shippingCost;

  const handleProceedToCheckout = () => {
    if (selectedItems.length === 0) return;
    if (typeof window !== "undefined") {
      localStorage.setItem(checkoutSelectionKey, JSON.stringify(selectedItems));
    }
              ...
              <div className="space-y-6">
                {cartItems.length === 0 ? (
                  <div className="py-12 text-center">
                    ...
                    <Link
                      href="/"
                      className="inline-block mt-4 px-6 py-3 bg-gray-900 text-white font-semibold hover:bg-gray-800 transition"
                    >
                      Continue Shopping
                    </Link>
                  </div>

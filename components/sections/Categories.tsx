        ...
        <section className="w-full max-w-[1737px] mx-auto px-4 sm:px-6 lg:px-4 py-8 md:py-12">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-4 sm:gap-6 md:gap-8">
                ...
                {categories.length > 0 && categories.map((category, index) => (
                    ...
                    >
                        {/* Optimized Image */}
                        <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 lg:w-auto lg:h-full  bg-white rounded-lg border border-gray-200 flex items-center justify-center group-hover:bg-gray-200 transition-colors duration-300 overflow-hidden">
                            <Image
                                src={category.image!}
                                alt={category.name}
                                width={100}
